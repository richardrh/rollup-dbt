...
def test_add_dialsup_matches_factor_formula():
    """dialsup == raw loss * forecast * euws, with no FX or uplift."""
    tag = "202601"
    ylt = pl.DataFrame({
    ...
    assert out["dialsup"][0] == pytest.approx(1000.0 * 1.05 * 0.95)
    assert out["dialsup"][1] == pytest.approx(500.0 * 1.10 * 1.00)
    assert out["dialsup"][2] == pytest.approx(250.0 * 1.00 * 0.80)
...
def test_dialsup_uses_requested_forecast_tag():
    ...
    expected = [800.0 * 1.10 * 0.90, 200.0 * 1.20 * 1.00]
