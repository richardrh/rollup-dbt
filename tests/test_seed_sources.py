...
def test_load_discovers_nested_csv_and_parquet_by_stem(tmp_path: Path) -> None:
...
def test_load_duplicate_stems_raise_clear_error(tmp_path: Path) -> None:
    data_root = tmp_path / "data"
...
def test_load_absent_and_empty_roots_return_empty_dict(tmp_path: Path) -> None:
    assert load(tmp_path / "missing") == {}

    data_root = tmp_path / "data"
    (data_root / "seeds").mkdir(parents=True)
    assert load(data_root) == {}
