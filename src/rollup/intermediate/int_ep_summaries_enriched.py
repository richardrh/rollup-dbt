...
class Model(PolarsModel[[pl.LazyFrame, dict[str, pl.LazyFrame]]]):
    ...
    @classmethod
    def _transform(
        cls, ep_summaries: pl.LazyFrame, seeds: dict[str, pl.LazyFrame]
    ) -> pl.LazyFrame:
        if "lobs" not in seeds:
            raise ValueError(
                "int_ep_summaries_enriched: input 'seeds' missing required key 'lobs'"
            )
        if "perils" not in seeds:
            raise ValueError(
                "int_ep_summaries_enriched: input 'seeds' missing required key 'perils'"
            )
