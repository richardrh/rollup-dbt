...
| doc | what it covers |
|---|---|
| [`data-requirements.md`](data-requirements.md) | **The contract for a real run.** YLT wire schemas, every seed CSV, currency-derivation rule, forecast-factor join contract, failure-mode reference table. |
...
| [`calculations.md`](calculations.md) | Polars stage modules that compute the loss chain, with the original SQL quoted. Also: `apply_rollup_scope`, the `chain.CHAIN` registry walker, EP curves, the dialsup funnel, and a per-stage status summary table. |

## Also relevant (outside docs/)

- [`../data/seeds/README.md`](../data/seeds/README.md) — per-seed schema, source, and population status.
...
- [`../polars/RH-TODO-DATA.md`](../polars/RH-TODO-DATA.md) — concrete TODO list for collecting real data.
